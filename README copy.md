...
또는 터미널에서:
```bash
./scripts/server-control.sh start    # 서비스 시작
./scripts/server-control.sh stop     # 서비스 중지
./scripts/server-control.sh deploy   # 수동 배포 트리거
```

### 수동 배포
`git push` 후 자동 배포가 안 될 경우:
```bash
./scripts/server-control.sh deploy
```
또는 CLI에서 직접 실행 시 반드시 `set -a`로 환경변수를 export해야 합니다:
```bash
